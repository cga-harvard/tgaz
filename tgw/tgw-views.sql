...
ftype.name_en ftype_en, snote.src_note_ref snote_ref, snote.full_text snote_text, snote.uri snote_uri
...
AND sp_tr.trsys_id != 'na' AND sp_tr.default_per_type = 1;

-- outer joins are needed since spellings are not always there for a given type
CREATE OR REPLACE VIEW v_children AS
SELECT po.*, pn.sys_id child_sys_id, sp_vn.written_form child_vn, sp_tr.written_form child_tr
FROM part_of po JOIN  placename pn ON (po.child_id = pn.id)
LEFT JOIN spelling sp_vn ON (pn.id = sp_vn.placename_id)
LEFT JOIN spelling sp_tr ON (pn.id = sp_tr.placename_id)
WHERE sp_vn.script_id > 0 AND sp_vn.default_per_type = 1
AND sp_tr.trsys_id != 'na' AND sp_tr.default_per_type = 1;
